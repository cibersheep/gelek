...
Page {
    ...
    function formatText(json) {
        ...
        console.log("++--- Gen: " + response.input[0].gen + " : " + response.gen)
    }
    ...
    function sendCommandToGtk() {
        if (command.text !== "") {
            if (response.input[0].type === "char") { //Make char input accepted from TextField
                launcher.sendCommand('{ "type":"char", "gen":' + response.input[0].gen + ', "window":' + response.input[0].id + ', "value": "' + command.text.slice(0) + '" }')
                console.log("+---- Gen: " + response.input[0].gen + " : " + response.gen)
            } else {
